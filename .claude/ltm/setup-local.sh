#!/bin/bash
#
# LTM Local Setup Script - Sets up Long-Term Memory using a locally-built container
#
# Prerequisites:
#   1. Clone the LTM repository and set LTM_HOME:
...
#
#   3. Run this script from your target project:
#      cd /path/to/your/project
#      bash $LTM_HOME/.claude/ltm/setup-local.sh
#
# This script:
#   1. Creates project-specific container name and ports
#   2. Creates the LTM container from local image (ltm-mcp-server)
#   3. Registers MCP with Claude Code
#   4. Copies slash commands (/remember, /recall, /forget, /ltm)
...
echo -e "${BLUE}╔════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   LTM - Local Container Installation       ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════╝${NC}"
echo
...
check_ltm_home() {
    if [ -z "$LTM_HOME" ]; then
        echo -e "${RED}Error: LTM_HOME environment variable not set.${NC}"
        echo
        echo "Please set LTM_HOME to the claude-ltm repository directory:"
        echo "  export LTM_HOME=/path/to/claude-ltm"
        echo
        echo "Then build the container:"
        echo "  podman build -t ltm-mcp-server \$LTM_HOME/.claude/ltm/"
        exit 1
    fi

    if [ ! -d "$LTM_HOME/.claude/ltm" ]; then
        echo -e "${RED}Error: LTM_HOME does not appear to be a valid claude-ltm directory.${NC}"
        echo "Expected: $LTM_HOME/.claude/ltm"
        exit 1
    fi

    echo -e "${GREEN}✓ LTM_HOME: $LTM_HOME${NC}"
}
...
check_prerequisites() {
    ...
    if ! $CONTAINER_CMD image exists ltm-mcp-server 2>/dev/null; then
        echo -e "${RED}✗ Local image 'ltm-mcp-server' not found${NC}"
        echo
        echo "Build the container first:"
        echo "  $CONTAINER_CMD build -t ltm-mcp-server \$LTM_HOME/.claude/ltm/"
        missing=1
    else
        echo -e "${GREEN}✓ Local image 'ltm-mcp-server' found${NC}"
    fi
...
# Copy slash commands from LTM_HOME
copy_commands() {
    echo -e "${BLUE}Copying slash commands...${NC}"

    for cmd in remember.md recall.md forget.md ltm.md; do
        cp "$LTM_HOME/.claude/commands/$cmd" ".claude/commands/$cmd"
    done
    echo -e "${GREEN}✓ Slash commands copied${NC}"
}
...
create_container() {
    ...
    # Create the container using local image
...
if [ ! -f "$CFG" ]; then
    echo "Error: server.json not found. Run setup-local.sh first." >&2
    exit 1
fi
...
print_summary() {
    echo
    echo -e "${GREEN}╔════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║      LTM Local Setup Complete!             ║${NC}"
    echo -e "${GREEN}╚════════════════════════════════════════════╝${NC}"
    echo
    echo -e "${BLUE}Container:${NC}"
    echo "  Name:   $CONTAINER_NAME"
    echo "  Image:  ltm-mcp-server (local)"
