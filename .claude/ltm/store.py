...
class MemoryStore:
    ...
    def create(
        ...
        for tag in memory_data["tags"]:
            if tag not in index.get("tag_index", {}):
                index.setdefault("tag_index", {})[tag] = []
            if memory_id not in index["tag_index"][tag]:
                index["tag_index"][tag].append(memory_id)
    ...
    def update(self, memory_id: str, **fields: Any) -> bool:
        ...
        if memory_id in index["memories"]:
            ...
            # Handle tag changes
            if "tags" in fields:
                old_tags = set(index_entry.get("tags", []))
                new_tags = set(fields["tags"])

                # Remove from old tag index entries
                for tag in old_tags - new_tags:
                    if tag in index.get("tag_index", {}):
                        if memory_id in index["tag_index"][tag]:
                            index["tag_index"][tag].remove(memory_id)
                        if not index["tag_index"][tag]:
    ...
    def delete(self, memory_id: str, archive: bool = True) -> bool:
        ...
        if memory_id in index["memories"]:
            ...
            tags = index["memories"][memory_id].get("tags", [])
            for tag in tags:
                if tag in index.get("tag_index", {}):
                    if memory_id in index["tag_index"][tag]:
                        index["tag_index"][tag].remove(memory_id)
